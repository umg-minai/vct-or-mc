...
d <- dist(scale(agcs[model_columns]), method = "euclidean")
agcs$cluster <- fixed_sized_clustering(d, size = 20)
agcs <- agcs[order(agcs$cluster, agcs$or),]

knitr::kable(
    agcs[c("cluster", "or", "ncases", "weight", model_columns)],
    digits = 2
)
...
```{r rcvglmnet, echo = FALSE, message = FALSE, warning = FALSE}
y <- agcs[, "weight"]
x <- model.matrix(
weight ~ 0 +
    sum.dura.lama.nonlap.tiva +
...
m <- rcv.glmnet(y = jitter(y), x = x, nfolds = 10, intercept = FALSE)
knitr::kable(
    cbind(
        as.matrix(coef(m, s = "lambda.1se")),
        as.matrix(coef(m, s = "lambda.min"))[, 1]
    ),
