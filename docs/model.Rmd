...
crf <- merge(crf, setting, all = TRUE)

crf$water.trap[is.na(crf$water.trap)] <- 0

weights <- read.csv(find_git_root_file("raw-data", "zeosys", "weights.csv"))
```
...
## remove later and use true weight
#agcs$weight <- 400
agcs$agc.id <- rownames(agcs)
agcs <- merge(
    agcs, weights[c("agc.id", "available", "weight1")],
    by = "agc.id", all.x = TRUE
)
agcs$weight <- agcs$weight1

## limit to available agcs
agcs <- agcs[as.logical(agcs$available),]
...
agcs <- agcs[!agcs$or %in% c("09-01", "09-03"),]
agcs <- agcs[agcs$ncases >= 10,]
agcs$water.trap <- crf[["water.trap"]][match(agcs$agc.id, crf$agc.id)]
